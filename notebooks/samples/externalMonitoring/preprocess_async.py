...
def build_df(df_input: pd.DataFrame, df_output: pd.DataFrame):
    base_completa = (
    ...
    df_input['SAFRA_REF'] = df_input['SAFRA_REF'].astype(str)

    return (base_completa[features+['SAFRA_REF']], df_output)
...
def get_shap(data, model_path):
  ...
  # so it does not break host validation
  df_shap['SAFRA_REF'] = 0
  df_shap['mlops_dtref'] = 0
